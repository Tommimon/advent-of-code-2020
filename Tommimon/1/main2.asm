...
MAIN:   la		        $s3, END		        # 
        li              $s4, 2020
        la		        $s0, ARRAY		        # $s0 is i
LOOP1:  beq		        $s0, $s3, EXIT	        # if $s0 == $s3 then EXIT
        addi	        $s1, $s0, 4		        # $s1 is j
LOOP2:  beq		        $s1, $s3, ENDLOOP1
        addi            $s2, $s1, 4             # $s2 is k
LOOP3:  beq		        $s2, $s3, ENDLOOP2
        lw		        $t0, 0($s0)
        lw		        $t1, 0($s1)
        lw		        $t2, 0($s2)
        add		        $t3, $t0, $t1		    
        ...
        bne             $t3, $s4, ENDLOOP3
        mult            $t0, $t1                # do first mult
        mflo            $a0
        mult            $a0, $t2                # do second mult
        mflo            $a0                     # $a0 = $t0 * $t1 * $t2
        li		        $v0, 1
